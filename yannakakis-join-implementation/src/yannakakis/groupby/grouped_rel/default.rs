...
//! Assumes there are no NULL values in the keys.
...
pub struct DefaultNonSingularGroupedRel {
...
impl DefaultNonSingularGroupedRel {
...
impl GroupedRel for DefaultNonSingularGroupedRel {
...
#[inline]
/// Validate that each column in `keys` is not nullable.
fn validate_lookup_keys(keys: &[ArrayRef]) {
    assert!(keys.iter().all(|col| !col.is_nullable()));
}

/// A [GroupedRelBuilder] implementation that produces a [DefaultNonSingularGroupedRel].
pub struct DefaultNonSingularGroupedRelBuilder {
...
impl GroupedRelBuilder for DefaultNonSingularGroupedRelBuilder {
    ...
    fn finish(
        ...
        let grouped_rel: DefaultNonSingularGroupedRel = DefaultNonSingularGroupedRel::new(
            schema,
...
impl DefaultNonSingularGroupedRelBuilder {
...
pub struct DefaultSingularGroupedRel {
    /// The scheme of the [GroupedRel]
    pub schema: NestedSchemaRef,
...
impl DefaultSingularGroupedRel {
...
impl GroupedRel for DefaultSingularGroupedRel {
    fn schema(&self) -> &NestedSchemaRef {
        &self.schema
    }
...
pub struct DefaultSingularGroupedRelBuilder {
    /// The index of the columns in the [SemijoinResultBatch]s that will be used as the groupby-key.
    group_on_cols: Vec<usize>,
...
impl GroupedRelBuilder for DefaultSingularGroupedRelBuilder {
    fn build(&mut self, batches: &[SemiJoinResultBatch]) {
    ...
    fn finish(
        ...
        let grouped_rel = DefaultSingularGroupedRel::new(
            schema,
            std::mem::take(&mut self.map),
            std::mem::take(&mut self.group_keys),
            std::mem::take(&mut self.random_state),
...
impl DefaultSingularGroupedRelBuilder {
...
pub fn make_builder(
    ...
    if is_singular {
        Box::new(DefaultSingularGroupedRelBuilder::new(
            group_on_cols,
            keys_schema,
            capacity,
            random_state,
        ...
        Box::new(DefaultNonSingularGroupedRelBuilder::new(
            group_on_cols,
            keys_schema,
            capacity,
            random_state,
