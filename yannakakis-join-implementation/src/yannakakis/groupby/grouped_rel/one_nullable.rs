...
//! The key column is allowed to be nullable.
//! If the key column is not nullable, please use the faster [super::one] module.
...
pub struct OneKeyNullableNonSingularGroupedRel<T>
where
...
impl<T> GroupedRel for OneKeyNullableNonSingularGroupedRel<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
...
#[inline]
/// Validate that `keys` consist of a single column of primitive type.
...
impl<T> OneKeyNullableNonSingularGroupedRel<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
    ...
    pub fn lookup_impl(
        ...
        // Strangely enough, storing the hash value in a separate buffer and then using it in the actual 'lookup' loop is faster
        ...
        let null_count = keys.null_count();
        hashes_buffer.resize(keys.len(), 0);

        if null_count == 0 {
            // No need to check null values!
            ...
            // There are null-values in the key column: skip them!
            for (hash, src_key) in hashes_buffer.iter_mut().zip(keys.iter()) {
                if let Some(src_key) = src_key {
                    *hash = src_key.hash_one(&self.random_state);
                }
            }

            for (i, (hash, key)) in hashes_buffer.iter().zip(keys.iter()).enumerate() {
                if let Some(key) = key {
                    let hash = *hash;
                    let entry = self.map.get(hash, |(k, _, _)| *k == key);
    ...
    pub fn lookup_sel_impl(
        ...
        if null_count == 0 {
            ...
            // There are null values in the key column: skip them!
            let null_buffer = keys
                .nulls()
                .expect("Null buffer should exist when null_count() > 0")
                .inner();

            for (hash, row_nr) in hashes_buffer.iter_mut().zip(sel.iter()) {
                let not_null = unsafe { null_buffer.value_unchecked(row_nr) };
                if not_null {
                    let key = unsafe { keys.value_unchecked(row_nr) };
                    *hash = key.hash_one(&self.random_state);
                }
...
pub struct OneKeyNullableNonSingularGroupedRelBuilder<T>
where
    T: ArrowPrimitiveType,
{
...
impl<T> GroupedRelBuilder for OneKeyNullableNonSingularGroupedRelBuilder<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
    ...
    fn finish(
        ...
        let grouped_rel: OneKeyNullableNonSingularGroupedRel<T> =
            OneKeyNullableNonSingularGroupedRel::new(
                schema,
...
impl<T> OneKeyNullableNonSingularGroupedRelBuilder<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
    ...
    fn ingest(
        ...
        offset: usize,
        keys_and_hashes_buffer: &mut Vec<(Option<T::Native>, u64)>,
    ) {
        match batch {
    ...
    fn ingest_flat_batch(
        ...
        // may have something to do with the fact that each individual loop becomes simpler, and therefore easier to optimize / execute?
        keys_and_hashes_buffer.clear();
        keys_and_hashes_buffer.resize(keys.len(), (None, 0));

        let null_count = keys.null_count();
        if null_count == 0 {
            // No need to access null buffer for this batch
            for ((key, hash), src_key) in
                keys_and_hashes_buffer.iter_mut().zip(keys.values().iter())
            {
                *key = Some(*src_key);
                *hash = src_key.hash_one(&self.random_state);
            }
            ...
            for (row, (key, hash)) in keys_and_hashes_buffer.iter().enumerate() {
                self.update_key(key.unwrap(), 1, row, offset, *hash);
            }
        } else {
            // There are null-values in the key column
            for ((key, hash), src_key) in keys_and_hashes_buffer.iter_mut().zip(keys.iter()) {
                if let Some(src_key) = src_key {
                    *key = Some(src_key);
                    *hash = src_key.hash_one(&self.random_state);
                }
    ...
    fn ingest_nested_batch(
        ...
        if null_count == 0 {
            ...
            for (row, ((key, hash), weight)) in
                keys_and_hashes_buffer.iter().zip(weights).enumerate()
            {
                self.update_key(key.unwrap(), *weight, row, offset, *hash);
            }
        } else {
            // There are null-values in the key column
            for ((key, hash), src_key) in keys_and_hashes_buffer.iter_mut().zip(keys.iter()) {
...
pub struct OneKeyNullableSingularGroupedRel<T>
where
    T: ArrowPrimitiveType,
{
...
impl<T> GroupedRel for OneKeyNullableSingularGroupedRel<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
...
impl<T> OneKeyNullableSingularGroupedRel<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
...
pub struct OneKeyNullableSingularGroupedRelBuilder<T>
where
    T: ArrowPrimitiveType,
{
    /// The index of the column in the [SemijoinResultBatch]s that will be used as the groupby-key.
...
impl<T> GroupedRelBuilder for OneKeyNullableSingularGroupedRelBuilder<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
    ...
    fn finish(
        ...
        let grouped_rel: OneKeyNullableSingularGroupedRel<T> =
            OneKeyNullableSingularGroupedRel::new(
                schema,
                std::mem::take(&mut self.map),
                std::mem::take(&mut self.random_state),
                use_alternative,
...
impl<T> OneKeyNullableSingularGroupedRelBuilder<T>
where
    T: ArrowPrimitiveType,
    T::Native: HashValue,
{
    ...
    fn ingest_flat_batch(
        ...
        if null_count == 0 {
            ...
            for (key, hash) in keys_and_hashes_buffer.iter() {
                self.update_key(key.unwrap(), 1, *hash);
            }
        } else {
            // There are null-values in the key column
            for ((key, hash), src_key_opt) in keys_and_hashes_buffer.iter_mut().zip(keys.iter()) {
                if let Some(src_key) = src_key_opt {
                    *key = Some(src_key);
                    *hash = src_key.hash_one(&self.random_state);
                }
            }
    ...
    fn ingest_nested_batch(
        ...
        if null_count == 0 {
            ...
            for ((key, hash), weight) in keys_and_hashes_buffer.iter().zip(weights) {
                self.update_key(key.unwrap(), *weight, *hash);
            }
        } else {
            // There are null-values in the key column
            for ((key, hash), src_key) in keys_and_hashes_buffer.iter_mut().zip(keys.iter()) {
...
macro_rules! builder_one_key_nullable_helper_singular {
    ($key_type:ty, $group_col:expr, $capacity:expr, $random_state:expr, $flag:expr) => {
        Box::new(OneKeyNullableSingularGroupedRelBuilder::<$key_type>::new(
            $group_col,
            $capacity,
            $random_state,
            $flag,
...
macro_rules! builder_one_key_nullable_helper_non_singular {
    ($key_type:ty, $group_col:expr, $capacity:expr, $random_state:expr, $flag:expr) => {
        Box::new(
            OneKeyNullableNonSingularGroupedRelBuilder::<$key_type>::new(
                $group_col,
                $capacity,
                $random_state,
                $flag,
...
/// Create a new [GroupedRelBuilder] for a single primitive-typed, **nullable** key column.
pub fn make_builder(
    t: &DataType,
    group_on_col: usize,
    is_singular: bool,
    ...
    if is_singular {
        downcast_primitive! {
            t => (builder_one_key_nullable_helper_singular, group_on_col, capacity, random_state, flag),
            _ => panic!("make_builder for single primitive-typed, nullable key column expects primitive key")
        }
    } else {
        downcast_primitive! {
            t => (builder_one_key_nullable_helper_non_singular,  group_on_col, capacity, random_state, flag),
            _ => panic!("make_builder for single primitive-typed, nullable key column expects primitive key")
        }
    }
}
